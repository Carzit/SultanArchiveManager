...
def delete_window(root, app:ArchiveApp):
    if app.archive_manager:
        app.archive_manager.stop_watching()
    app.config_manager.configs.update({"watch_path":app.watch_path_entry.get(), 
                                       "archive_path":app.archive_path_entry.get(), 
                                       "save_path":app.save_path_entry.get(), 
                                       "monitor_interval":float(app.monitor_interval_entry.get()), 
                                       "max_archives":int(app.max_archives_entry.get()),
                                       "auto_start": app.auto_start_var.get()})
    root.destroy()
    sys.exit(-1)
